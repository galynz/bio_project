...
class MutationsSummary(object):
    def __init__(self, cancer_csv_paths_dict, cancer_clinical_paths_dict):
        self.ids_dict = {}
        self.mutations_dict = {}
        for cancer, paths_list in cancer_csv_paths_dict.items():
            for path in paths_list:
                self.add_csv_data(path, cancer)
        for cancer, paths_list in cancer_clinical_paths_dict.items():
            for path in paths_list:
                self.add_clinical_data(path, cancer)
        
    def add_csv_data(self, path,cancer):
        with open(path) as f:
            file_dict = csv.DictReader(f, dialect=csv.excel_tab)
            cancer_dict = self.ids_dict.setdefault(cancer, {})
            cancer_mutations_dict = self.mutations_dict.setdefault(cancer,{})
            for row in file_dict:
                ...
                #center = row["Center"]
                sample = cancer_dict.setdefault(patient_barcode, Sample(patient_barcode, tumor_barcode, norm_barcode))
                #sample.add_center(center)
                sample.add_mutation(mutation)
                
                mutation_obj = cancer_mutations_dict.setdefault(mutation, Mutation(mutation))
    ...
    def add_clinical_data(self, path, cancer):
        ...
        sample = self.ids_dict[cancer].get(patient_barcode, None)
        if sample:
    ...
    def write_output(self, output_path):
        with open(output_path, "w") as f:
            ...
            for cancer, cancer_ids in self.ids_dict.items():
                for sample in cancer_ids.values():
    ...
    def write_mutation_load_output(self, output_path):
        ...
        with open(output_path, "w") as f:
            ...
            csv_file.writeheader()
            for cancer, cancer_mutations_dict in self.mutations_dict.items():
                patients_num = len(self.ids_dict[cancer].keys())
                top_mutation_load_num = len([i for i in self.ids_dict[cancer].values() if i.top_mutation_load])
                top_patients_mutations_sum = sum([i.count_mutations() for i in self.ids_dict[cancer].values() if i.top_mutation_load])
                patients_mutations_sum = sum([i.count_mutations() for i in self.ids_dict[cancer].values()])
                lower_patients_mutations_sum = patients_mutations_sum - top_patients_mutations_sum
                for mut in cancer_mutations_dict.values():
    ...
    def find_high_mutation_load_patients(self):
        for cancer, cancer_ids_dict in self.ids_dict.items():
            patients_list = sorted(cancer_ids_dict.values(), key=lambda x: x.count_mutations(), reverse=True)
            stop_index = int(len(patients_list)/TOP_PERCENTIL)
            for patient in patients_list[:stop_index]:
                patient.update_top_mutation_load()
                
                
    def write_survival_output(self, output_path):
        with open(output_path, "w") as f:
            ...
            for cancer, cancer_ids_dict in self.ids_dict.items():
                days_dict = {}
                group_count = {}
                for sample in cancer_ids_dict.values():
                    group = sample.get_group()
